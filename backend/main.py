...
# ========== MOTOR CSP ==========
# En main.py, actualiza el endpoint /generar
...
async def generar_horario(request: HorarioRequest, db: Session = Depends(get_db)):
    ...
    logger.info(f"Cursos recibidos: {len(request.cursos)}")
    logger.info(f"Aulas recibidas: {len(request.aulas)}")
    logger.info(f"Docentes recibidos: {len(request.docentes)}")
    
    # Validar que hay datos
    if len(request.cursos) == 0:
        logger.error("No hay cursos para generar horario")
        raise HTTPException(400, "No hay cursos para generar horario")
    ...
    try:
        ...
        logger.info(f"CSP completado: {len(resultado['horario_generado'])} asignaciones")
    except Exception as e:
        logger.error(f"Error en CSP: {str(e)}")
        raise HTTPException(500, f"Error en el motor CSP: {str(e)}")
    
    # Guardar en base de datos (opcional)
    try:
        if resultado['estadisticas']['total_no_asignados'] == 0 and resultado['horario_generado']:
