...
### ⚙️ Requisitos No Funcionales (RNF) y Métricas
Para garantizar la calidad del sistema, el prototipo cumple con las siguientes métricas de validación:

* **Rendimiento:** El motor CSP de optimización debe procesar y generar el horario validando todas las restricciones en un tiempo **≤ 2 segundos** para una malla curricular estándar.
* **Escalabilidad:** La arquitectura soporta la carga de múltiples facultades. La base de datos relacional (MySQL/SQLite) permite la concurrencia sin bloqueo de tablas durante las consultas del motor.
* **Eficiencia:** El uso de CPU se mantiene optimizado gracias a la técnica de poda en la satisfacción de restricciones, evitando desbordamientos de memoria en el servidor.
* **Soporte Antigravity:** Interfaz de usuario de carga rápida (Zero-Gravity) con validaciones asíncronas para evitar el envío de formularios con conflictos al backend.

---

### 🧪 Ejecución de Pruebas (TDD)
El sistema ha sido construido bajo el enfoque **Test-Driven Development (TDD)** (Ciclo Red-Green-Refactor). Contamos con pruebas automatizadas que verifican la integridad del motor CSP, la asignación sin solapamientos y el manejo de errores.

**Para ejecutar las pruebas y validar la cobertura localmente:**
1. Abre la terminal en el directorio raíz (donde se encuentra `manage.py`).
